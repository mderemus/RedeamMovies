...
export default {
	...
	methods: {
		...
		saveMovie() {
			...
			if (this.movie.id) {
				// this.movie.inventoryStatus = this.movie.inventoryStatus.value ? this.movie.inventoryStatus.value: this.movie.inventoryStatus;
				...
				this.$toast.add({severity:'success', summary: 'Successful', detail: 'Movie Updated', life: 3000});
					...
					this.movie.title = this.movie.title.value;
					this.movie.description = this.movie.description.value;
		...
		filterCallback(){
			console.log("Test filter1: " + this.filters.global);
			//this.$refs.dt.filters = this.filters.global;
		},
		checkDateFilterType(){
